...
export default function LiveCallCounter({ locale }: Props) {
  ...
  // Server-rendered HTML (and reduced-motion / no-JS users) shows the final
  // numbers; the count-up only kicks in from the IntersectionObserver.
  const [calls, setCalls] = useState(TARGETS.calls);
  const [searches, setSearches] = useState(TARGETS.searches);
  const [reviews, setReviews] = useState(TARGETS.reviews);
  ...
  return (
    <>
      <style>{`
        ...
        .lcc-eyebrow { font-family:var(--font-sans); font-size:var(--text-sm); font-weight:600; color:var(--color-ink-muted); margin:0 0 var(--space-5); text-align:center; }
