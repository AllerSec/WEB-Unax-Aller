...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const titles: Record<string, string> = {
    es: "Diseñador Web en Vitoria-Gasteiz | Unax Aller",
    en: "Web Designer in Vitoria-Gasteiz | Unax Aller",
    eu: "Web Diseinatzailea Gasteizen | Unax Aller",
  };
  const descriptions: Record<string, string> = {
    es: "Diseñador web freelance para negocios de Vitoria-Gasteiz. Webs a medida con SEO local, diseño premium y rendimiento. Desde 1.300€ IVA incluido.",
    en: "Freelance web designer for businesses in Vitoria-Gasteiz. Custom websites with local SEO, premium design and performance. From €1,300 VAT included.",
    eu: "Web diseinatzaile freelance Gasteizko negozioetarako. Neurrizko webguneak SEO lokalarekin, diseinu premiuma eta errendimenduarekin. 1.300€-tik BEZ barne.",
  };
  ...
  return {
    ...
    openGraph: buildOpenGraph({ locale, title, description, path: "/disenador-web-vitoria" }),
    twitter: buildTwitter({ title, description }),
  };
}

export default async function VitoriaPage({ params }: Props) {
  const { lang } = await params;
  const locale = lang as "es" | "en" | "eu";
